...
/*RWD*/
        if(first){
            printf("AssembleSoundChunk: s->nchannels = %d\n",s->nchannels);
            printf("globalStreamWidth = %d\n",globalStreamWidth);
            first = 0;
        }
        startPos = p->startPos;
        endPos = p->endPos;
...
playCmd(Sound *s, Tcl_Interp *interp, int objc,	Tcl_Obj *CONST objv[])
{
    ...
    switch(s->encoding){
    case LIN16:
        printf("playCmd(): encoding %d = LIN16\n",s->encoding);
        break;
    case LIN24:
        printf("playCmd(): encoding %d = LIN24.\n",s->encoding);
        break;
    case SNACK_FLOAT:
        printf("playCmd(): encoding %d =  floats.\n",s->encoding);
        break;
    default:
        printf("playCmd(): encoding %d = something else.\n",s->encoding);
        break;
    }
...
/* RWD don't want  this! */
   // rate = 44100;
    ...
    s->writeStatus = WRITE;
    /*RWD*/
    printf("calling AudioOpen(): encoding = %d.\n",s->encoding);
    /* RWD ~should~ be s->encoding? */
    if (SnackAudioOpen(&ado, interp, s->devStr, PLAY, rate, devChannels,
                       LIN16 /*s->encoding*/) != TCL_OK) {
        wop = IDLE;
        s->writeStatus = IDLE;
        return TCL_ERROR;
    }
