...
if(evlogfile){
    static int first = 1;
    if(first){
        fprintf(evlogfile, "GetSample: encoding = %d\n",s->encoding);
        first = 0;
    }
}
#endif
...
/*RWD*/     val = infoPtr->buffer[index-infoPtr->filePos];
/* at this stage, we have integer values in floats */
//            printf("(2): %f\n",val);
            return /*(infoPtr->buffer[index-infoPtr->filePos]);*/ val;
        } else {
            infoPtr->eof = 1;
            return(0.0f);
        }
